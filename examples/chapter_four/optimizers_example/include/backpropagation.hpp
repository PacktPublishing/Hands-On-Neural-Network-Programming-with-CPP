...
class Backpropagation
{
  ...
  public:
    ...
    void update(const std::vector<Matrix> &dWperLayer, const std::vector<Matrix> &dBperLayer, int epoch)
    {
        auto &layers = net.getLayers();
        int layerIndex = 0;
        std::for_each(layers.begin(), layers.end(), [&dWperLayer, &dBperLayer, &layerIndex, this, &epoch](const Layer &layer) {
    ...
    {
        int msePeriod = 100;
        Matrix result(1, maxEpochs / msePeriod);  
        int epoch = 0;
