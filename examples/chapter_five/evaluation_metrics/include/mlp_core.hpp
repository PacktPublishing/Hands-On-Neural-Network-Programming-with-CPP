...
public:
  Layer(std::unique_ptr<ActivationFunction> activationFunction, Matrix initialWeights, Vector initialBiases) : 
      activationFunction(std::move(activationFunction)), weights(std::move(initialWeights)), biases(std::move(initialBiases))
  {
  ...
  virtual ~Layer() {}
  Layer(Layer const &o) : Layer(o.activationFunction->clone(), o.weights, o.biases) {}
