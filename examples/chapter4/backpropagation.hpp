...
class Backpropagation
{
  ...
  public:
    Backpropagation(MultilayerPerceptron &net, Dataset &trainingDataset, Dataset &validationDataset, double learningRate, int maxEpochs, int batchsize) : 
        net(net), trainingDataset(trainingDataset), validationDataset(validationDataset), learningRate(learningRate), maxEpochs(maxEpochs),
        batchsize(batchsize) {
    ...
    Matrix train()
    {
        ...
        while (epoch++ < maxEpochs)
        {
            ...
            if(epoch % msePeriod == 1) {
                ...
                auto validationCost = mse(net, validationDataset);
                std::cout << epoch << "\t" << trainingCost << "\t" << validationCost << "\n";
                result(0, epoch / msePeriod) = trainingCost;
                result(1, epoch / msePeriod) = validationCost;
            }
        }
        return result;
    }
