 ...
 * Chapter 3, example 4
 * 
 * Training a 4-layer network for the Iris dataset with softmax
 * 
 * 
**/

ann::LogisticActivationFunction g;
ann::SoftmaxActivationFunction softmax;
...
{
    Matrix cost = output.binaryExpr(expected, [](const double y, const double t){
        return -t * log(y + e);
    });
 
    double result = cost.sum();
    return (result / expected.cols()); 
}

double cost_derivate(const double expected, const double output)
{
    return -expected / (output + e);
}
...
backward(const Vector &x, const Vector &t, const Matrix &z1, const Matrix &z2, const Matrix &z3, 
    ...
    Matrix zDerivate = softmax.prime(z3);
    auto delta3 = zDerivate * sigma;
