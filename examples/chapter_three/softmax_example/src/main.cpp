 ...
 * Chapter 3, example 6
 * 
 * Backpropagation with softmax activation
 * 
 * 
**/
...
ann::MultilayerPerceptron initializeNetwork(double initializationRange)
{
    ...
    ann::Layer outputLayer(std::unique_ptr<ann::ActivationFunction>(new ann::SoftmaxActivationFunction()), wOut, Vector::Zero(3));
...
int main(int, char **)
{
    ...
    ann::Backpropagation<ann::LogCostFunction> backpropagation(net, irisDS, learningRate, epochs);
