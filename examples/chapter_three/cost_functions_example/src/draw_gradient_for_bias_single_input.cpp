 ...
 * Chapter 3, example 3
 * 
 * Find gradient vectors for the bias parameter
 * 
 * 
**/
...
long slice = static_cast<long>((0.50 - wMin) * n / (wMax - wMin));
...
void generateChartSurface(mglGraph &gr, const mglData &a, double ang1, double ang2, double zMin, double zMax)
{
    ...
    long i = slice;
    double w = ((wMax - wMin) * i / n) + wMin;
    mglData x(m);
    mglData y(m);
    x.Fill(w, w);
    y.Fill(bMin, bMax);
    mglData z(a.SubData(i));
    gr.Plot(x, y, z, "2r");
}
...
void generateChart(const Matrix &data, const Matrix &gradients, double zMin, double zMax)
{
    ...
    generateChartW("b", gr, _2DData, 0, 0.3, gradFromX, gradFromY, gradX, gradY);

    gr.WriteFrame("gradient_bias.svg");
}
...
int main(int, char **)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
            ...
            if ((i == slice) && (j % gradTick == 0))
            {
                int gradIndex = j / gradTick;

                if (gradIndex < gradientsNumber)
                {
                    double parcialDerivate = derivate(xt, w, b);
                    gradients(gradIndex, 0) = b;
